...
/* Menu_2*/
static const Menu_typedef Main_Game_Dinosaur[1] = {{
    "Dinosaur",
    Main_Game,
    NULL,
    Func_Dinosaur_run,
...
static const Menu_typedef Main_Settings_Fontsize[1] = {{
    "Fontsize",
    Main_Settings,
    NULL,
    Func_Fontsize_set,
...
static const Menu_typedef Main_Settings_ColorMode[1] = {{
    "ColorMode",
    Main_Settings,
    NULL,
    Func_ColorMode_set,
...
static const Menu_typedef Main_Settings_RefreshRate[1] = {{
    "Refresh Rate",
    Main_Settings,
    NULL,
    Func_RefreshRate_set,
    0,
}};
static const Menu_typedef Main_Settings_MirrorFlipHo[1] = {{
    "MirrorFlip-Ho",
    Main_Settings,
    NULL,
    Func_MirrorFlipHo_set,
    0,
}};
static const Menu_typedef Main_Settings_MirrorFlipVer[1] = {{
    "MirrorFlip-Ver",
    Main_Settings,
    NULL,
    Func_MirrorFlipVer_set,
    0,
}};
static const Menu_typedef Main_Hello_Sayhello[1] = {{
    "Sayhello",
    Main_Hello,
    NULL,
    Invalid_Operation,
...
static const Menu_typedef Main_Hello_Smile[1] = {{
    "Smile",
    Main_Hello,
    NULL,
    Func_Smile_run,
...
void Func_ColorMode_enter(void) {
    ...
    if (colormode_set == White) {
        OLED_ShowString_Rowcentering(40, " White ", 16, 1);
    } else {
        OLED_ShowString_Rowcentering(40, " Black ", 16, 1);
    }
    OLED_Refresh_Poll();
}
...
void Func_MirrorFlipHo_enter(void) {
    ...
    if (mirrorflipHo_set == No) {
        OLED_ShowString_Rowcentering(40, " Horizontal: No ", 16, 1);
    } else {
        OLED_ShowString_Rowcentering(40, " Horizontal:Yes ", 16, 1);
    }
    OLED_Refresh_Poll();
}
...
void Func_MirrorFlipVer_enter(void) {
    OLED_BufferClear();
    OLED_ShowString_Rowcentering(16, " [Mirror Flip] ", 16, 1);
    if (mirrorflipVer_set == No) {
        OLED_ShowString_Rowcentering(40, " Vertical: No ", 16, 1);
    } else {
        OLED_ShowString_Rowcentering(40, " Vertical:Yes ", 16, 1);
    }
    OLED_Refresh_Poll();
}
...
void Menu_Handler(void) {
    ...
    if (Menu_Pointer == Main) {
        ...
        property = Menu_Pointer->Parent[Get_MenuIndex(Menu_Pointer)].Child_Menuproperty;
    }
    if (KEY_num == Zero) {
        if (property == Menu_Loop) {
            Jumped_key = Zero;
            KEY_Loop_pressed();
        }
    } else {
...
void Switch_Menu(void) {
    ...
    while (' ' <= *(Menu_Pointer->Name + i) && *(Menu_Pointer->Name + i) <= '~') {
        i++;
    } // 获取名字长度
    len = i;
...
void Draw_Menu(uint8_t headmenu_y, uint8_t submenu_y, uint8_t len, uint8_t n, uint8_t cur_y, uint8_t rec_y, uint8_t height) {
    ...
    // 显示菜单头
    for (i = 0; i < len + 2; i++) {
        OLED_ShowChar(i * Mysize[fontsize].size_title / 2, headmenu_y, ' ', Mysize[fontsize].size_title, 0);
    }
...
void KEY_Parent_next(void) {
    ...
    if (Menu_Pointer == Main) {
        nodes_number = Main_Child_nodesnumber;
    }
...
void KEY_Parent_previous(void) {
    ...
    if (nodes_number <= Mysize[fontsize].row_number) {
        ...
        if (UserChoose <= Current_showrange) { // 到达显示上边界
            ...
            {
                Current_showrange = nodes_number - Mysize[fontsize].row_number;
                UserChoose = nodes_number - 1;
            } else {
                Current_showrange--;
                UserChoose--;
...
void Func_Dinosaur_enter(void) {
    ...
    OLED_ShowString(2, 1, "Highest Score:", 12, 1);
    OLED_ShowNum_Dec(86, 1, highest_score, 5, 12, 1);
...
void Func_Dinosaur_run(void) {
    switch (Game_State) {
    case Game_Stop:
        Dinosaur_Stop_Handler();
        break;
    case Game_Run: /* HAL_GPIO_TogglePin(GPIOC,GPIO_PIN_13); */
        Dinosaur_Run_Handler();
        break;
    case Game_Over:
        Dinosaur_Over_Handler();
        break;
    default:
        break;
    }
...
void Dinosaur_Run_Handler(void) {
    ...
    if ((tree1_x <= (dino_right - ((jump < 5) ? jump : 5)) && tree1_x > (DINO_INIT_X + 1) && (dino_y + dino_back_legImg.height) >= (BASE_LINE_Y - tree1->height)) || (tree_x <= (dino_right - ((jump < 5) ? jump : 5)) && tree_x > (DINO_INIT_X + 1) && (dino_y + dino_back_legImg.height) >= (BASE_LINE_Y - tree->height))) {
        ...
        if (score >= highest_score) {
            highest_score = score;
            OLED_ShowString(0, 2, "New Hst", 12, 1);
        }
        ...
        score = score_raw / 5;
        if (jump != 0) {
            Dinosaur_MoveDino(dino_y, -1);
        } else {
            Dinosaur_MoveDino(dino_y, (leg > 4) ? 1 : 0);
            if (++leg > 9) {
                leg = 0;
            }
        }
...
void Dinosaur_MoveDino(int16_t y, int type) {
    switch (type) {
    case 0:
        OLED_ShowPicStruct(DINO_INIT_X, y, dino_front_legImg, 1);
        break;
    case 1:
        OLED_ShowPicStruct(DINO_INIT_X, y, dino_back_legImg, 1);
        break;
    case -1:
        OLED_ShowPicStruct(DINO_INIT_X, y, dino_jumpsImg, 1);
        break;
    case -2:
        OLED_ShowPicStruct(DINO_INIT_X, y, dino_crashedImg, 1);
        break;
    default:
        return;
    }
...
void Dinosaur_MoveTree(void) {
    if (Game_State == Game_Run) {
        ...
        if (interval > 0 && interval <= 46)
            ...
            tree1_x += interval;
        // 判断是否作出树
        if (tree_x + tree->width <= 127) {
            OLED_ShowPicStruct(tree_x, BASE_LINE_Y - tree->height, *tree, 1);
        }
        if (tree1_x + tree1->width <= 127) {
            OLED_ShowPicStruct(tree1_x, BASE_LINE_Y - tree1->height, *tree1, 1);
        }
    } else {
        if (tree_x + tree->width <= 127) {
            OLED_ShowPicStruct(tree_x, BASE_LINE_Y - tree->height, *tree, 1);
...
void Dinosaur_DisplayScore(int score) {
    switch (Game_State) {
    case Game_Over:
        OLED_ShowString(50, 1, "Score:", 12, 1);
        OLED_ShowNum_Dec(86, 1, score, 5, 12, 1);
        OLED_ShowString(2, 14, "Highest Score:", 12, 1);
        OLED_ShowNum_Dec(86, 14, highest_score, 5, 12, 1);
        break;
    case Game_Stop:
        OLED_ShowString(50, 1, "Score:", 12, 1);
        OLED_ShowNum_Dec(86, 1, score, 5, 12, 1);
